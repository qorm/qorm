...
func writeWAV(path string, samples []float64) error {
	...
	if err := binary.Write(f, binary.LittleEndian, uint32(36)+dataSize); err != nil { return err }
	...
	if err := binary.Write(f, binary.LittleEndian, uint32(sampleRate*2)); err != nil { return err }
	if err := binary.Write(f, binary.LittleEndian, uint16(2)); err != nil { return err }
	if err := binary.Write(f, binary.LittleEndian, uint16(16)); err != nil { return err }
...
func squareWave(dur, f float64) []float64 {
	...
	for i := 0; i < n; i++ {
		...
		if math.Sin(2*math.Pi*f*t) >= 0 { out[i] = 1 } else { out[i] = -1 }
	}
	return out
}
...
func adsr(samples []float64, a, r float64) []float64 {
	...
	for i := 0; i < n; i++ {
		var env float64
		switch {
		case i < aN: env = float64(i) / float64(aN)
		case i > n-rN: env = float64(n-i) / float64(rN)
		default: env = 1
		}
		samples[i] *= env
	}
...
func mix(parts ...[]float64) []float64 {
	max := 0
	for _, p := range parts { if len(p) > max { max = len(p) } }
	out := make([]float64, max)
	for _, p := range parts { for i, v := range p { out[i] += v } }
	for i := range out { out[i] = math.Tanh(out[i] * 0.7) }
	return out
}
...
func shootSFX() []float64 {
	...
	for i := 0; i < n; i++ {
		...
		if math.Sin(2*math.Pi*f*t) >= 0 { out[i] = 0.6 } else { out[i] = -0.6 }
	}
...
func deathSFX() []float64 {
	notes := []struct{ midi int; dur float64 }{
...
func musicLoop() []float64 {
	...
	for i := range durs { durs[i] = 0.5 }

	var leadBuf []float64
	for i, n := range lead {
		if n == 0 { leadBuf = append(leadBuf, silence(durs[i]*beat)...); continue }
		leadBuf = append(leadBuf, adsr(squareWave(durs[i]*beat, freq(n)), 0.01, 0.06)...)
	}
	var bassBuf []float64
	for i, n := range bass {
		if n == 0 { bassBuf = append(bassBuf, silence(durs[i]*beat)...); continue }
...
func main() {
	...
	if len(os.Args) > 1 { outdir = os.Args[1] }
	if err := os.MkdirAll(outdir, 0o755); err != nil { panic(err) }
	files := []struct {
		name string
		gen  func() []float64
	}{
	...
	for _, f := range files {
		if err := writeWAV(filepath.Join(outdir, f.name), f.gen()); err != nil { panic(err) }
	}
}