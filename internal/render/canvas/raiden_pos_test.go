...
func TestRaidenPlayerLoads(t *testing.T) {
	...
	for y := 400; y < 448; y++ {
		for x := 144; x < 192; x++ {
			...
			if absi(int(c.R),60)<25 && absi(int(c.G),130)<25 && absi(int(c.B),230)<25 { blue++ }
			if int(c.R) < 90 && int(c.G) > 100 && int(c.B) > 180 && c.A > 150 { blue++ }
			if absi(int(c.R),229)<12 && absi(int(c.G),229)<12 && absi(int(c.B),234)<12 { grey++ }
		}
	}
