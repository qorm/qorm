...
// list drawing into an image.RGBA. It is stateless — all pipeline state is
// scoped to a single Render call.
//
// Known limitations (scheduled milestones, see ADR-0007): paints use draw.Src
// (no alpha blending over siblings), text ignores clip/opacity, clips replace
// rather than intersect, shadows are stacked rects without blur.
type SoftwareRenderer struct{}

// Render implements Renderer.
func (SoftwareRenderer) Render(ops *op.Ops, img *image.RGBA) {
	// Every frame starts from a clean white background.
