...
var nodeSchema = [][4]string{
	...
	{"d", "string", "SVG path data string for vector paths (the `path` widget)", "矢量路径的 SVG 路径数据字符串(`path` 组件)"},
...
var stepFields = [][4]string{
	...
	{"async", "bool", "`http.*`: run the request in the background — the dispatch returns immediately (so the frame at its boundary already shows the loading state and the session stays responsive) and the result branch runs when the reply arrives. Defaults to `false`, which blocks the dispatch; it also falls back to `false` on a host with no background sink, so the same JSON stays portable", "`http.*`:在后台发起请求——派发立即返回(因此其边界处的那一帧已显示 loading 状态,会话全程可用),结果分支在响应到达时执行。默认 `false`,即阻塞派发;宿主未安装后台汇时同样退化为 `false`,保证同一份 JSON 可移植"},
	{"key", "string", "`http.*`: name a request slot — starting a new request on a key cancels whichever request was still open on it AND discards that one's outcome entirely (no `result`/`error` write, no branch). This is what makes search-as-you-type land the reply to the LAST keystroke instead of whichever round trip finished last. Only an `async` request can be superseded; unkeyed requests never cancel each other", "`http.*`:给请求命名一个槽位——同 key 的新请求会取消该槽位上仍在途的旧请求,并**整体丢弃**它的结果(不写 `result`/`error`,不跑分支)。这正是「搜索框每输入一次都发请求」时保证落地的是**最后一次**输入的结果、而不是最后返回的那一次。只有 `async` 请求会被取代;不带 key 的请求之间互不取消"},
	{"timeout", "number", "`http.*`: this request's ceiling in milliseconds, overriding the shared client's 20s. Expiry is an ordinary failure — the `error` path is written and `onError` runs, with the message `request timed out after <n>ms`. Applies to synchronous requests too. Omit (or 0) to keep the 20s ceiling", "`http.*`:该请求的超时上限(毫秒),覆盖共享客户端的 20 秒默认值。超时按普通失败处理——写入 `error` 路径并执行 `onError`,错误信息为 `request timed out after <n>ms`。同步请求同样生效。不写(或写 0)则保持 20 秒上限"},
	{"pending", "string", "`http.*`: a state path held `true` for exactly as long as the request is open — set on launch, cleared when it settles, INCLUDING on failure, timeout and refusal. Replaces the hand-written pair of `state.set` steps (the one that reliably forgets the error path); bind a spinner to `{{ state.<path> }}` as usual. Reference-counted, so overlapping requests on one path hold it until the last settles and a superseded request cannot switch off its successor's spinner", "`http.*`:一个状态路径,在请求在途期间恒为 `true`——发起时置真,结束时复位,**失败、超时、被上限拒绝时同样复位**。用它替代手写的那对 `state.set`(手写版最容易漏掉错误路径);转圈组件照常绑定 `{{ state.<路径> }}`。该路径按引用计数,多个请求共用一个标志时要等最后一个结束才复位,被取代的请求也不会关掉其后继的转圈"},
	{"ms", "number", "`delay`: how long to wait, in milliseconds. The steps FOLLOWING the delay in the same list run when it expires", "`delay`:等待的毫秒数。同一列表中位于该步骤**之后**的步骤会在等待结束后执行"},
	{"onSuccess", "array", "`http.*`: steps run after a 2xx response; the decoded response is bound as `{{ response }}` (the `result` path is written first). With `async` they run in the completion callback, after the dispatch has ended: `{{ state.x }}` reads live, the action's args stay frozen at dispatch time", "`http.*`:2xx 响应后执行的步骤;解码后的响应绑定为 `{{ response }}`(`result` 路径先写入)。配合 `async` 时它们在完成回调里运行(派发已结束):`{{ state.x }}` 读取当前值,动作参数则冻结在派发时刻"},
	{"onError", "array", "`http.*`: steps run after a failure; the message is bound as `{{ error }}` (the `error` path is written first). With `async` they run in the completion callback, on the same terms as `onSuccess`", "`http.*`:失败后执行的步骤;错误信息绑定为 `{{ error }}`(`error` 路径先写入)。配合 `async` 时同样在完成回调里运行,规则与 `onSuccess` 相同"},
...
func stepDesc(typ string) (en, zh string) {
	m := map[string][2]string{
		...
		"delay":              {"wait `ms` milliseconds, then run the steps that FOLLOW it in the same list — `render` / `delay` / `render` paces a staged reveal declaratively. It never blocks: the wait goes to the host's background sink, and on a host with no sink (an offline render, an MCP simulation) it degrades to no wait at all, so the action still reaches the same final state", "等待 `ms` 毫秒,然后执行同一列表中位于其**后面**的步骤——`render` / `delay` / `render` 即可声明式地编排分段展示。它不会阻塞:等待交给宿主的后台汇执行;宿主未安装后台汇时(离线渲染、MCP 模拟)退化为完全不等待,动作仍然到达同样的最终状态"},
		...
		"state.toggle":       {"flip a boolean, or a `field` on a matched array element; on a scalar array toggles membership of `match`", "翻转布尔值,或匹配数组元素上的某个 `field`;对标量数组则切换 `match` 的成员资格"},
