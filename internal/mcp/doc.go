...
var toolDescriptionsZH = map[string]string{
	...
	"qorm_query":           "查找与选择器匹配的节点，条件可任选：type、textContains、idContains、hasProp（多个条件通过 AND 组合）。返回每个匹配项的 id、类型、标签和祖先路径。在应用补丁 (patch) 前使用此工具定位节点。只读。",
	...
	"qorm_measure":         "精确解析实时渲染：返回每个组件的用户表达（类型、文本、状态绑定）与实际渲染结果 —— x,y,w,h, visible，以及计算出的 color/background/fontSize/fontWeight/padding/borderRadius/border/opacity/zIndex/position/x-overflow；canvas 行还可能带 hostLimits（本宿主无法兑现的样式键或占位 widget，例如 fontFamily 被忽略、webview 占位）。测量由当前渲染宿主提供：原生 canvas 窗口导出其保留模式渲染图，浏览器/WebView 则回报 DOM。需要带渲染窗口/客户端的运行中应用；单独通过 stdio 运行 `qorm mcp` 没有渲染宿主。如需无窗口、确定性的单次 canvas 测量，请使用 CLI `qorm measure`。用于查看人类用户的实时宿主究竟渲染了什么。",
	"qorm_validate":        "在打补丁或保存之前，将 QORM 场景节点或整个应用对照组件模式 (schema)、组件目录类型规则、表达式语法和设计令牌约束进行校验。返回 valid（布尔值）以及诊断警告或错误数组。",
	"qorm_check_layout":    "根据预期校验实时渲染；返回每个检查项的通过/失败状态以及实际值。`checks` 是由 {id, <assertions>} 组成的数组。断言：visible(bool) | type(组件类型 string) | text(组件必须包含的子字符串，与所表达或渲染后的文本匹配) | noOverflow(bool, 无水平溢出) | minW|maxW|minH|maxH(px 数值) | x|y(px 数值, ±3 容差) | within(id: 该盒子必须位于该 id 盒子的内部) | below(id: 必须在该 id 的下方开始) | backgroundNot|colorNot(必须不存在的子字符串 —— 例如在暗黑模式下断言 \"255, 255, 255\" 即非白色) | role(渲染后的 ARIA role 字符串) | hasAriaLabel(bool) | contrastRatio(最小 WCAG 对比度，如 AA 正文 4.5)。浏览器/WebView 的 DOM 回报包含渲染器隐式注入的 role 和计算对比度；canvas 图回报派生的 role/accessibleName，并在祖先背景为不透明纯色时计算 WCAG 对比度（渐变/图片/子树透明度仍设 contrastUnavailable）。canvas 行可能带 hostLimits（被忽略的样式键或占位 widget）。示例：[{\"id\":\"wifi\",\"type\":\"switchlisttile\",\"visible\":true,\"within\":\"settings\"},{\"id\":\"chart\",\"noOverflow\":true}]。明确失败：无法识别的断言键（如拼写错误）会判定为失败，未被测量的 within/below 目标 id 会以 'not found' 判定为失败 —— 绝不会有检查项被静默通过。需要带渲染窗口/客户端的运行中应用：原生 canvas 检查使用其保留模式渲染图，浏览器/WebView 检查使用 DOM 回报；单独通过 stdio 运行 `qorm mcp` 没有渲染宿主。可选的 viewportW/viewportH（px）在校验前设置运行时视口，使响应式 `when` 分支按该窗口尺寸解析 —— 注意测得的矩形仍来自宿主的真实窗口（活动浏览器客户端也会在下次加载/缩放时覆盖该视口）。",
}
...
func ToolsMarkdownZH() string {
	...
	b.WriteString("**宿主窗口**：应用打开时的窗口由 `qorm.json` 旁的可选文件 `qorm.config.json` 配置（宿主/构建期配置 —— 不会被打进 bundle，也不参与签名），并以 `qorm.json` 内的 `display` / `platforms.desktop.window` 作为清单内回退：`width`/`height`（初始尺寸）、`title`、`resizable`（默认 true）、`chromeless`（无标题栏/边框）、`transparent`（透明背景 → 异形/自定义形状窗口）、`hideLog`/`hideTray`。`qorm.config.json` 优先于清单；`qorm_inspect` 会回报解析后的最终取值。\n\n")
	b.WriteString(toolsByCategoryZH)
	...
	b.WriteString("\n带有 `*` 标记的参数为必填项；其余为选填项。\n\n")
	b.WriteString(toolsExamplesZH)
	return b.String()
}

// toolsByCategory is a fixed, hand-curated grouping of the 25 tools by intent.
// It is static text (not derived from a map) so the generated page is
// deterministic. Keep it in sync with toolList() — TestMCPDocInSync re-renders
// the page from toolList(), so a new tool that lands in the table but not here
// is a review signal, not a silent drift.
const toolsByCategory = `## Tool categories
...
// toolsExamples shows representative JSON-RPC calls per category. The request
// shape is the standard MCP tools/call over stdio (` + "`qorm mcp`" + `) or HTTP
// (` + "`/mcp`" + ` on a running ` + "`qorm run`" + `).
const toolsExamples = `## Examples
