...
var All = []Cap{
	...
	{Stem: "volume", Widget: "volume", Ops: []string{"volumeGet", "volumeSet", "volumeUp", "volumeDown"}, Callback: "qormOnVolume", Platforms: []string{IOS, Android, Mac, Linux}, Desc: "System output volume.", Notes: "Android volumeSet pending (get/up/down only)"},
	...
	{Stem: "notify", Widget: "notify", Ops: []string{"notify"}, Callback: "qormOnNotify", Platforms: []string{IOS, Mac, Linux, Windows, Web}, Desc: "Local notification.", Notes: "Android falls back to the Web Notification API; Windows shows a balloon (toast planned)"},
	{Stem: "badge", Widget: "dockbadge", Ops: []string{"badge"}, Callback: "", Platforms: []string{IOS, Mac}, Desc: "App icon / Dock badge count."},
	{Stem: "screenshot", Widget: "screenshot", Ops: []string{"screenshot"}, Callback: "qormOnScreenshot", Platforms: []string{IOS, Android, Mac, Linux, Web}, Desc: "Capture the screen / app view.", Notes: "Linux needs grim, scrot or ImageMagick; Windows pending"},
	...
	{Stem: "securestorage", Widget: "securestorage", Ops: []string{"secureSet", "secureGet"}, Callback: "qormOnSecure", Platforms: []string{IOS, Android, Mac, Windows, Web}, Desc: "Secure key/value storage (iOS Keychain / Android Keystore / Windows DPAPI).", Notes: "web falls back to localStorage (not hardware-encrypted); Linux keyring pending"},
...
var notesZH = map[string]string{
	...
	"notify":        "Android 会回退到 Web Notification API；Windows 为气泡通知（Toast 升级计划中）",
	"screenshot":    "Linux 需要 grim、scrot 或 ImageMagick；Windows 待完成",
	...
	"securestorage": "Web 端会回退到 localStorage（无硬件加密）；Linux 密钥环待完成",
