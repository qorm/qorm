// Package miniapp is the mini-program (小程序) static export target. A
...
//
// This target is static export only: a one-shot export of the app's initial UI.
// There is no on-device QORM runtime — no live session, no state/actions/bindings
// evaluation, no measure/self-verify, no action dispatch. Tap targets are wired
// (bindtap + data-h) but onTap only logs a static-export notice; actions do not
// run in mini-programs. Full interactivity is roadmap (v0.3, not committed).
package miniapp
...
func BuildProject(rt *runtime.Runtime) map[string]string {
	...
	return map[string]string{
		...
		"app.wxss":               themeWXSS(rt.App.Theme, rt.App.DesignTokens),
...
func themeWXSS(theme string, tokens map[string]model.DesignToken) string {
	// Theme palettes come from internal/render (the single source of truth).
	// WeChat WXSS supports CSS custom properties. "auto" pins to the light
	// palette here — WeChat dark mode is app.json-level, roadmap.
	page := "page {\n  " + render.ThemeVarsFor(theme) + "\n  background:var(--bg); color:var(--label);\n}"
	if css := render.TokenCSS("page", tokens); css != "" {
		page += "\n" + css
	}
...
func pageJS() string {
	return `// generated by QORM — static export.
// The UI is rendered statically from the QORM app's initial state; tap targets
// carry their handler index in data-h, but there is no on-device QORM runtime:
// actions, bindings, and state do not run in mini-programs.
Page({
  data: {},
  onTap: function (e) {
    var h = e.currentTarget.dataset.h;
    console.log('[qorm] static export — actions do not run in mini-programs (tap handler', h, ')');
  }
})
`
}
