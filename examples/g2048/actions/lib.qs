# lib.qs — the shared 2048 core: the reserved library file the loader collects
# as type:"scriptlib" and the runtime prepends to EVERY script action at
# dispatch. Keep this file to fn definitions and comments — it is spliced
# ahead of each action's body, so any top-level statement here would run
# before every action.
#
# The board is a flat 4x4 row-major array of tile values (0 = empty);
# state.view holds each cell's exponent (0 = empty, else log2 of the value)
# so the scene can index state.colors / state.labels without a pow() builtin.
...
# float64, and the only randomness the game gets — the game never reads a
# clock, so a given seed always plays the same sequence).
fn spawn() {
  let empty = []
  for i in range(16) {
