...
- `POST /update {"source": "https://example.com/app.qorm.bundle"}` —— 拉取、
  ...
  source 的拉取带有 SSRF 防护:私网(RFC 1918 / RFC 4193)、链路本地
  (覆盖 `169.254.169.254` / `fe80::` 云元数据地址)、组播及未指定地址一律
  拒绝。检查在 DNS 解析之后、建立连接时执行,因此重定向跳转或 DNS 重绑定
  的域名都绕不过去;重定向最多跟随 5 跳,且只允许 http(s)。回环地址与本地
  文件路径仍然放行 —— 本机上的 Bundle 服务器是正常的开发工作流,而回环
  并不会给本地调用方增加它本来没有的可达性。
