...
带 `--update-url` 打包时必须同时携带 `--trust <key.pub>`——两个 flag 强制成对。信任划分是刻意设计的:

- **随包 payload**(.ipa / .apk / PWA 内置的 bundle.json)经安装渠道获得信任
  ——商店签名、TLS 来源——与交付 WASM 运行时本体的是同一渠道,启动时不重复验签。
- **所有 OTA 来源的 bundle**(从更新服务器拉取,或从本地存储恢复的历史更新)
  在激活前都用内嵌信任公钥做 ed25519 验签;验签失败即丢弃并逐级回落
  (上一次更新 → 随包 payload)。失败的更新永远不会触碰运行中的应用。

## CLI 自更新信任

`qorm update` 只有在完成两步校验后才替换二进制:先用运行中二进制内嵌的公钥
验证 release 的 `SHA256SUMS` 清单的 ed25519 签名,再用清单比对下载文件的
sha256。签名缺失或无法验证时默认拒绝(`--insecure-skip-verify` 是显式逃生口);
内嵌公钥列表支持轮换(过渡版本同时内嵌新旧两把)。
