...
| `volume` | `volume` | volumeGet<br>volumeSet<br>volumeUp<br>volumeDown | `qormOnVolume` | ios, android, mac, linux | 系统输出音量。 (Android 的 volumeSet 待完成（仅支持读取/加减）) |
...
| `notify` | `notify` | notify | `qormOnNotify` | ios, mac, linux, windows, web | 本地通知。 (Android 会回退到 Web Notification API；Windows 为气泡通知（Toast 升级计划中）) |
| `badge` | `dockbadge` | badge | `—` | ios, mac | 应用图标 / Dock 徽标计数。 |
| `screenshot` | `screenshot` | screenshot | `qormOnScreenshot` | ios, android, mac, linux, web | 捕获屏幕 / 应用视图。 (Linux 需要 grim、scrot 或 ImageMagick；Windows 待完成) |
...
| `securestorage` | `securestorage` | secureSet<br>secureGet | `qormOnSecure` | ios, android, mac, windows, web | 安全键值存储（iOS Keychain / Android Keystore）。 (Web 端会回退到 localStorage（无硬件加密）；Linux 密钥环待完成) |
