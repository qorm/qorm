...
| `screenrecord` | `screenrecord` | screenRecordStart<br>screenRecordStop | `qormOnScreenRecord` | ios, android, mac, web | 录制屏幕。 (Android 使用 MediaProjection（Android 14+ 需要类型为 mediaProjection 的前台服务）) |
...
| `orientation` | `orientation` | lockOrientation | `—` | ios, android, web | 锁定屏幕方向（竖屏/横屏）。 |
| `videocapture` | `videocapture` | recordVideo | `qormOnVideo` | ios, android, web | 用摄像头录制视频。 (Android 使用系统相机 Intent（ACTION_VIDEO_CAPTURE）) |
