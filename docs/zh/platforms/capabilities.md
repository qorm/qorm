...
| `volume` | `volume` | volumeGet<br>volumeSet<br>volumeUp<br>volumeDown | `qormOnVolume` | ios, android, mac, linux | 系统输出音量。 |
| `brightness` | `brightness` | brightnessGet<br>brightnessSet<br>brightnessUp<br>brightnessDown | `qormOnBrightness` | ios, android, mac | 屏幕亮度。 (桌面端目前仅支持 macOS) |
...
| `notify` | `notify` | notify | `qormOnNotify` | ios, mac, linux, web | 本地通知。 (Android 会回退到 Web Notification API) |
| `badge` | `dockbadge` | badge | `—` | ios, mac | 应用图标 / Dock 徽标计数。 |
| `screenshot` | `screenshot` | screenshot | `qormOnScreenshot` | ios, android, mac, web | 捕获屏幕 / 应用视图。 |
...
| `share` | `share` | share | `qormOnShare` | ios, android, mac, web | 打开系统分享面板。 |
...
| `securestorage` | `securestorage` | secureSet<br>secureGet | `qormOnSecure` | ios, android, mac, web | 安全键值存储（iOS Keychain / Android Keystore）。 (Web 端会回退到 localStorage（无硬件加密）) |
...
| `tts` | `tts` | speak<br>speakStop | `—` | ios, android, mac, linux, web | 文字转语音（大声朗读字符串）。 |
...
| `screens` | `screens` | screens | `qormOnScreens` | mac, linux, windows | 枚举显示器。 |
